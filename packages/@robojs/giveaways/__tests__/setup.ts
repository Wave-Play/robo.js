...
import { mockFlashcore, mockClient, mockUlid, mockCron } from './helpers/mocks.js'
...
jest.mock('robo.js', () => {
  ...
  const Flashcore = {
    get: (key: string, options?: { namespace?: string[], default?: any }) =>
      mockFlashcore.get(key, options),
    set: (key: string, value: any, options?: { namespace?: string[] }) =>
      mockFlashcore.set(key, value, options),
    delete: (key: string, options?: { namespace?: string[] }) =>
      mockFlashcore.delete(key, options)
  }
...
// Mock @robojs/cron module (optional dependency)
...
// Suppress console output during tests (can be re-enabled per test)
// Uncomment the following to suppress console output globally:
/*
global.console = {
  ...console,
  log: jest.fn(),
...
declare global {
  namespace NodeJS {
    interface Global {
      testUtils: typeof import('./helpers/test-utils.js')
    }
  }
}
