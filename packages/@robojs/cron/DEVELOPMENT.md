...
```bash
npx robo add /path/to/plugin-cron
```

Replace `/path/to/plugin-cron` with the actual path to your plugin's directory. Remember to build your plugin between changes using the `robo build` command.
