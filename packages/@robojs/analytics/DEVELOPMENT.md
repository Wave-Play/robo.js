...
```bash
npx robo add /path/to/plugin-analytics
```

Replace `/path/to/plugin-analytics` with the actual path to your plugin's directory. Remember to build your plugin between changes using the `robo build` command.
