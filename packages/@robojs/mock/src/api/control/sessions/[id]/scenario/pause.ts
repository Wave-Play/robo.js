...
/**
 * POST /api/control/sessions/:id/scenario/pause - Pause scenario execution
 *
 * Request: {} (empty body)
 *
 ...
 *
 * Errors:
 * - 400: No scenario loaded or scenario not in 'running' status
...
export default async (request: RoboRequest) => {
	...
	// Pause the scenario
	try {
		const state = session.scenarioManager.getRunner().pause()
		return state
	} catch (error) {
		const message = error instanceof Error ? error.message : 'Failed to pause scenario'
