...
function resolveGlobalCommands(request: RoboRequest) {
	...
	return { session, sessionId, appId }
}
...
export const GET = define(
	...
	async (request) => {
		const resolved = resolveGlobalCommands(request as unknown as RoboRequest)
...
export const POST = define(
	{
		...
		description: 'Create a new global command. Returns 201 if a command with the same name does not already exist, or a 200 if it does (upsert)',
		tags: ['Application Commands'],
		params: z.object({
			app_id: z.string().describe('The application ID (Snowflake)')
		}),
	...
	async (request) => {
		const resolved = resolveGlobalCommands(request as unknown as RoboRequest)
		if (resolved instanceof Response) return resolved
		const { session, sessionId, appId } = resolved
