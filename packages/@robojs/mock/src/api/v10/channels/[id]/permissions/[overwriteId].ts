...
const PermissionOverwriteParams = z.object({
	id: z.string().describe('Channel ID (Snowflake)'),
	overwriteId: z.string().describe('Overwrite ID (Snowflake)')
})

const SetPermissionOverwriteBodySchema = z.object({
	type: z.number().int().nullable().optional().describe('Permission overwrite type (0 = Role, 1 = Member)'),
	allow: z.union([z.string(), z.number().int()]).nullable().optional().describe('Bitwise value of allowed permissions'),
	deny: z.union([z.string(), z.number().int()]).nullable().optional().describe('Bitwise value of denied permissions')
}).passthrough()
...
export const PUT = define(
	{
		...
		tags: ['Channels'],
		params: PermissionOverwriteParams,
		body: SetPermissionOverwriteBodySchema,
		response: {
			204: z.undefined()
		}
	},
	async (request) => {
		const resolved = resolveChannelForPermissions(request as unknown as RoboRequest)
