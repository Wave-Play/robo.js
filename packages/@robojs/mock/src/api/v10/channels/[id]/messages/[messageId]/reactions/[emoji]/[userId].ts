...
export const DELETE = define(
	{
		...
		params: z.object({
			...
			emoji: z.string().describe('URL-encoded emoji (custom format: name:id)'),
			userId: z.string().describe('User ID (Snowflake)')
		}),
		response: {
			204: z.undefined()
		}
	...
	async (request) => {
		...
		const { id: channelId, messageId, emoji, userId } = (request as unknown as RoboRequest).params as {
			id: string
			messageId: string
			emoji: string
			userId: string
