# 🛠️ Developing @roboplay/plugin-api
...
```bash
npm install /path/to/@roboplay/plugin-api
```

Replace `/path/to/@roboplay/plugin-api` with the actual path to your plugin's directory. Remember to build your plugin between changes using the `npm run build` command.
