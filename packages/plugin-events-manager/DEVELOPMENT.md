# 🛠️ Developing discord-events-manager
...
```bash
npx robo add /path/to/discord-events-manager
```

Replace `/path/to/discord-events-manager` with the actual path to your plugin's directory. Remember to build your plugin between changes using the `robo build` command.
