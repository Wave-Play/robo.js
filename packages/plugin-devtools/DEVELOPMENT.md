# 🛠️ Developing @roboplay/plugin-devtools
...
```bash
npx robo add /path/to/@roboplay/plugin-devtools
```

Replace `/path/to/@roboplay/plugin-devtools` with the actual path to your plugin's directory. Remember to build your plugin between changes using the `robo build` command.
