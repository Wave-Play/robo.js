...
import type { ManifestRoutes } from '../../types/manifest.js'
...
export async function generateManifest(
	...
	// Discover and process routes from the new plugin-based route system
	let routeEntries: ManifestRoutes | undefined
	if (type === 'robo') {
		routeEntries = await discoverAndProcessRoutes(plugins, config)
	}
...
/**
 * Discover and process routes from plugins and the project.
 * This is the entry point for the new plugin-based route system.
 */
async function discoverAndProcessRoutes(
	plugins: Map<string, PluginData>,
	config: Config
): Promise<ManifestRoutes | undefined> {
	const buildDir = config.experimental?.buildDirectory
		? path.join(process.cwd(), config.experimental.buildDirectory)
		: path.join(process.cwd(), '.robo', 'build')

	// 1. Discover route definitions from plugins and project
	const routes = await discoverRoutes(plugins)

	if (routes.length === 0) {
		logger.debug('No route definitions found')
		return undefined
	}

	// 2. Validate routes for conflicts
	...
	// 4. Process scanned entries
	const routeEntries = await processAllRoutes(scannedResults)

	// 5. Validate processed entries
	for (const [route, entries] of scannedResults) {
		const processedEntries = routeEntries[route.namespace]?.[route.name] ?? []
		const entryErrors = validateProcessedEntries(route, processedEntries)

		if (entryErrors.length > 0) {
			for (const error of entryErrors) {
				logger.error(error)
			}
			throw new Error(`Entry validation failed for ${route.namespace}:${route.name}`)
		}
	}

	// 6. Log summary
	let totalEntries = 0
	for (const namespace of Object.values(routeEntries)) {
		for (const entries of Object.values(namespace)) {
			totalEntries += entries.length
		}
	}
	logger.debug(`Processed ${totalEntries} route entries across ${routes.length} route(s)`)

	return Object.keys(routeEntries).length > 0 ? routeEntries : undefined
}
