# 🛠️ Developing @roboplay/plugin-modtools
...
```bash
npx robo add /path/to/@roboplay/plugin-modtools
```

Replace `/path/to/@roboplay/plugin-modtools` with the actual path to your plugin's directory. Remember to build your plugin between changes using the `robo build` command.
