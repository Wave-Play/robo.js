# 🛠️ Developing plugin-confessions
...
```bash
npm install /path/to/plugin-confessions
```

Replace `/path/to/plugin-confessions` with the actual path to your plugin's directory. Remember to build your plugin between changes using the `npx robo build` command.
