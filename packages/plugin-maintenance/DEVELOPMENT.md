# 🛠️ Developing @roboplay/plugin-maintenance
...
```bash
npm install /path/to/@roboplay/plugin-maintenance
```

Replace `/path/to/@roboplay/plugin-maintenance` with the actual path to your plugin's directory. Remember to build your plugin between changes using the `npm run build` command.
