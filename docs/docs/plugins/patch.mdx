...
<Terminal execute>{`robo add @robojs/patch`}</Terminal>

Or install it as a normal package if you're not using **Robo.js**:

<Terminal>{`npm install @robojs/patch`}</Terminal>
...
<CardContainer><Card href="https://github.com/Wave-Play/robo.js/tree/main/packages/@robojs/patch" title="🔗 GitHub Repository" description="Explore source code on GitHub."/><Card href="/ref/framework" title="🔗 API Reference" description="Check out the entire API."/></CardContainer>
