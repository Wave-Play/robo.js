...
<Terminal create>{`<project-name> -p @robojs/server`}</Terminal>
...
<CardContainer><Card href="https://github.com/Wave-Play/robo.js/tree/main/packages/plugin-api" title="🔗 GitHub Repository" description="Explore source code on GitHub."/><Card href="/ref/framework" title="🔗 API Reference" description="Check out the entire API."/></CardContainer>
