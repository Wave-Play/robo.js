...
Desire a Robo with a backstory, specific character traits, or a distinctive personality? Tell it a story with the `instructions` config:

```js
// config/plugins/robojs/ai.mjs
export default {
	instructions: 'You are Batman, protector of the Weeb City Discord server.'
}
```
...
export default {
	...
	instructions: 'You are a wise wizard of Webland!',
...
And guess what? This plugin includes native voice capabilities, letting your Robo chat in voice channels too!

## Voice Capabilities ️

Voice features are built right into `@robojs/ai`! Your Robo can talk in voice channels, just like a real person.

Need a different spoken persona or transcript language? Configure it right from your plugin file:
...
export default {
	...
	voice: {
		instructions: 'Keep replies short and friendly when speaking aloud.'
	}
}
```
...
- **Insights**: Ensure your `/documents` folder is correctly set up and contains **[valid text files](https://platform.openai.com/docs/assistants/tools/file-search/supported-files)**. You may need to explicitly tell your Robo to use the uploaded files in your `instructions`.
