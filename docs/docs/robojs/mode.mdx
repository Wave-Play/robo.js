...
<center>`.env.<mode>` > `.env`</center>
<center>`robo.<mode>.js` > `robo.js`</center>
<br/>
...
Plugin config files are also affected by modes the same way!

## Modules

You can also create mode-specific modules by adding a `.<mode>` suffix to the module folder name.

This lets you write code that is only loaded when a specific mode is active.
...
The `Mode` class has the following methods:

- `Mode.get()`: Returns the current mode.
- `Mode.is(mode: string)`: Returns `true` if the current mode is the specified mode.

This can be useful for conditionally loading features, or even for debugging purposes!

You cannot change the mode at runtime, but you can use the `--mode` flag to run a different mode from the command line.
