...
Sometimes you may need to store sensitive information, such as API keys, database URLs, or **[Discord Credentials](#)**. Hardcoding these values in your source code is a bad idea, as they can be easily exposed.

**Environment Variables** provide a way to store these values outside of your codebase. They're loaded into your project when it starts, and are less likely to be exposed accidentally.

## Standard File

**Robo** projects typically use a file called `.env`. This file is in the root of your project and can be used to store sensitive information.
...
Variables are stored as key-value pairs, separated by an equals sign (`=`). The key is the name of the variable, and the value is the sensitive information. You can then access these values in your code using `process.env`.
...
For those using **Git**, rest assured that **Robo** projects follow best practices out of the box with `.gitignore`.

:::tip

**[Doppler](https://www.doppler.com)** is awesome at managing your environment variables. It provides a secure way to store and manage your secrets. It's also great for teams, as it allows you to share secrets securely.
...
**Robo.js** manages your **Environment Variables** automatically, but it also exports an API for you to use.
...
You can change default behavior by passing an options object to `load()` or `loadSync()`.
