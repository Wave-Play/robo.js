...
<Callout>
When using upstream proxy mode or deploying auth to a separate origin:

- Ensure auth server cookies use `SameSite=None; Secure` for cross-origin
- Add the frontend origin to the CORS allowlist
- The client auto-fetches CSRF tokens before redirects
</Callout>
