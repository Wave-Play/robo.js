...
<Callout type="warn">
When the frontend and auth server run on different origins:

- Set cookies with `SameSite=None; Secure` on the auth server
- Add the frontend origin to your CORS allowlist
- Set `AUTH_REDIRECT_PROXY_URL` for OAuth callback handling
</Callout>
