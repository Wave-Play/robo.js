...
Returns `true` after the plugin's `_start` lifecycle hook has successfully set an engine. Returns `false` if no engine is configured or the hook hasn't run yet.
