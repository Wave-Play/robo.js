...
This detection runs in the plugin's `_start` lifecycle hook. If a custom `engine` is provided in the config, auto-detection is skipped entirely.
...
The plugin initializes during the Robo `_start` lifecycle event:
...
Enable the intent in your Robo config if you want this tracking:
