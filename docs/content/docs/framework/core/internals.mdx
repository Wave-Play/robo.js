...
Plugins use the same mechanism to hook into the lifecycle. Hook execution supports a priority system where hooks with the same priority run in parallel, and different priorities run sequentially. See [Lifecycle Hooks](./lifecycle) for the full reference.
