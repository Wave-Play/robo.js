...
For a full one-time migration of all legacy data, install `@robojs/flashcore-extras` and run the `/db legacy-migrate` terminal command:

```
/db legacy-migrate
```

This reads every key from `.robo/data/` and copies it to `.robo/flashcore/`, then disables the lazy read-through.
...
6. **Run `/db legacy-migrate`** (optional) — For a complete one-time migration of all legacy KV data.
7. **Clean up** — Once you have verified everything works, you can optionally remove the `.robo/data/` directory.

<Callout type="warn">
Do not delete `.robo/data/` until you have confirmed all keys have been migrated. The lazy migration only copies keys as they are read.
</Callout>
