...
Migrate all data from the legacy `.robo/data/` format to `.robo/flashcore/`. This is a one-time operation that reads every key from the old format and writes it to the new format.

```
/db legacy-migrate
```

See the [Migration Guide](/docs/framework/core/flashcore/migration-guide) for context on when to use this.
