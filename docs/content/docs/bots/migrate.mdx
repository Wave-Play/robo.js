...
Use the `_start` lifecycle event to run existing setup code alongside Robo handlers. This lets you migrate handlers incrementally.
...
New commands and events can use the Robo.js file structure while existing ones continue running through `_start`. As you migrate each handler to a file in `src/events/` or `src/commands/`, remove it from the `_start` event.
