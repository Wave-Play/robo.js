...
<Files>
  <Folder name="src/robo/cli/commands" defaultOpen disabled>
    ...
    <Folder name="inspect" defaultOpen>
      <File name="index.ts" note="npx robo inspect" />
      <File name="routes.ts" note="npx robo inspect routes" />
      <File name="state.ts" note="npx robo inspect state" />
    </Folder>
  </Folder>
</Files>
...
- `inspect/routes.ts` → `npx robo inspect routes`
- `inspect/index.ts` → `npx robo inspect` (parent command)
...
<Cards>
  <Card href="/docs/robo-cli/robo" title="Robo CLI Reference" description="Built-in command reference." />
