---
title: "listPrismaUsers()"
---

# Function: listPrismaUsers()

```ts
function listPrismaUsers(client, options): Promise<{
  page: number;
  pageCount: number;
  total: number;
  users: AdapterUser[];
...
Retrieves paginated AdapterUser records via Prisma. Loads complete
...
- Fetching full rows is heavier than [listPrismaUserIds](list-prisma-user-ids); use IDs when possible.
- Large page sizes can tax memory. Stick to DEFAULT\_LIST\_PAGE\_SIZE or smaller.
...
| ------ | ------ | ------ |
| `client` | [`PrismaClientLike`](../interfaces/prisma-client-like) | Prisma client with a `user` delegate. |
