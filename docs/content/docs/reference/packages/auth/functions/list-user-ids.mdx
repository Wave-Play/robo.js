...
```ts
function listUserIds(page): Promise<{
  ids: string[];
  page: number;
  pageCount: number;
  total: number;
...
you only need identifiers instead of full AdapterUser records.
...
default page size is DEFAULT\_INDEX\_PAGE\_SIZE (500 users). Fetching
