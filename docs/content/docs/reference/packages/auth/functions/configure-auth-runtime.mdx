...
| `options` | [`ConfigureAuthRuntimeOptions`](../interfaces/configure-auth-runtime-options) | Runtime values ([ConfigureAuthRuntimeOptions](../interfaces/configure-auth-runtime-options)). |

## Returns

`void`

Nothing; sets module-scoped state used by [getServerSession](get-server-session) and [getToken](get-token).
