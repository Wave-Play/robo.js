---
title: "configureAuthProxyRuntime()"
---
...
Performance:
- Every [getServerSession](get-server-session) call becomes an HTTP request; consider caching responses.
- Providing `options.secret` enables local JWT decoding via [getToken](get-token), reducing upstream calls by 50%+.
...
| ------ | ------ | ------ |
| `options` | [`ConfigureAuthProxyRuntimeOptions`](../interfaces/configure-auth-proxy-runtime-options) | See [ConfigureAuthProxyRuntimeOptions](../interfaces/configure-auth-proxy-runtime-options) for details. |
