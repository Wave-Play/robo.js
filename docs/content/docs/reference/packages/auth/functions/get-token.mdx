...
- `{ raw: false }` or omitted: returns the full Session object.
...
- Database strategy with `{ raw: false }` returns the full Session object, not a JWT.
...
JWT payload (JWT strategy), Session object (database strategy without raw), raw cookie string (with raw), or `null`.
