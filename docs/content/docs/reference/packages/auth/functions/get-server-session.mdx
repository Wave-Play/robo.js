...
Retrieves the active Auth.js Session using the configured runtime.
...
Edge cases:
- Throws if runtime is unconfigured ([configureAuthRuntime](configure-auth-runtime) or [configureAuthProxyRuntime](configure-auth-proxy-runtime) not called).
...
Session or `null` if no session is available.
