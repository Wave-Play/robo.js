# 🛠️ Developing starter-plugin-js
...
```bash
npx robo add /path/to/starter-plugin-js
```

Replace `/path/to/starter-plugin-js` with the actual path to your plugin's directory. Remember to build your plugin between changes using the `robo build` command.
